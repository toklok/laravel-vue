<script setup>
import { reactive, onMounted, onUnmounted, watchEffect, defineEmits, ref } from 'vue'
    ...
    if (value === '=') {
        try {
            if (math.evaluate(calculatorState.input)) {
                ...
                submitCalculation(math.evaluate(calculatorState.input).toString());
                return calculatorState.input = math.evaluate(calculatorState.input).toString();
            }
        } catch {
...
const handleKeyUpEvent = (ev) => {
    const button = buttons.find(btn => btn.value.toLowerCase() === ev.key.toLowerCase());
...
<template>
    <form @submit.prevent="">
        <div class="mx-auto overflow-hidden shadow-lg shadow-lg border rounded-lg px-5 pt-5 h-[600px]">
            <div>
                <input type="text" v-model="calculatorState.input"
                    class="p-5 block w-full mb-3 text-black text-right text-3xl" disabled />
                <div class="grid grid-cols-4 gap-4">
                    <button v-for="btn in buttons" :key="btn.value" @click="updateDisplay(btn.value)" :class="{
                        'col-span-2': btn.value === '0',
                        'h-20': true,
                        [btn.type ?? 'number']: btn.type || 'number',
                        'transition-colors border-2 duration-400 border-2 border-gray-500': activeButton === btn.value,
                        ...
                        {{ btn.value === 'C' && calculatorState.input === '' ? 'AC' : btn.value }}
                    </button>
                </div>
            </div>
        </div>
